...
function reasonForRegion(region: { prioridade: string; score: number }) {
  if (region.prioridade === "critica")
    return "Alta oportunidade com cobertura abaixo do potencial comercial.";
...
function Regs() {
  ...
  async function loadLeads() {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Não foi possível carregar regiões prioritárias.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const regions = useMemo(() => {
    const grouped = new Map<
      string,
      { cidade: string; uf: string; oportunidades: number; score: number; totalScore: number }
    >();
    leads.forEach((lead) => {
    ...
    <div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Importe CNPJs ou crie leads para gerar o ranking regional."
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {regions.map((r) => {
            ...
            return (
              <div
                key={r.cidade}
                className="overflow-hidden rounded-xl border border-[#DDE5EF] bg-white shadow-sm transition hover:shadow-md"
              >
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="text-[11px] uppercase tracking-wide text-[#64748B] font-semibold">
                        {r.uf} · Região monitorada
                      </div>
                      <h3 className="text-xl font-bold text-[#0B1F33] mt-1">{r.cidade}</h3>
                    </div>
                    <div className="text-right">
                      <div className="text-3xl font-bold tabular-nums" style={{ color: c }}>
                        {r.score}
                      </div>
                  ...
                  <div className="mt-3">
                    <PriorityBadge level={r.prioridade} />
                  </div>
                    ...
                    {[
                      ...
                      return (
                        <div key={m.l} className="rounded-lg bg-[#F8FAFC] p-3">
                          ...
                          <div className="text-lg font-bold text-[#0B1F33] mt-1 leading-none tabular-nums">
                            {m.v}
                          </div>
                  ...
                  <div className="mt-4 rounded-lg bg-[#F8FAFC] p-3">
                    <div className="text-[11px] uppercase font-bold text-[#64748B]">
